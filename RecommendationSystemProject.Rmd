...
#calculate the genre effect and penalize the groups with small number of ocurrences
...
y_j<- new_edx %>% 
  ...
  group_by(year_released) %>% 
  summarize(y_j= sum(rating-m_i--g_g-u_u-avg_rating)/ (n()+ lambda),.groups="drop") 
#calculate the year effect and penalize the groups with small number of ocurrences
...
predicted_ratings<-validation %>% 
  ...
  left_join(u_u, by="userId") %>% 
  left_join(g_g, by= "genres") %>% 
  left_join(y_j, by= "year_released") %>% 
  mutate(prediction= avg_rating+ m_i+g_g+ u_u+ y_j) %>% 
  .$prediction
...
```
This improved the RMSE even more with a result of 0.86441 with a penalty of 4.9.
...
```
These modeling approaches worked quite well. The final model was able to beat expectations with an RMSE of 0.86441.
